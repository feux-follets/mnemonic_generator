...
int main(int argc, char* argv[]) {
	...
	for (std::size_t i = 0; i < bit_size / 64; ++i) {
		std::uint64_t random_number{rng.rand64()};
		...
		for (std::size_t j = 0; j < 8; ++j)            // Parse 8-chars to hex representation
			hex_value += fmt::format("{:0>2x}", std::uint8_t(bytes[i * 8 + j]));
	}
	...
	for (const char& c : hex_value)
		bin_value += fmt::format("{:0>4b}", ff::hex_to_dec.find(c));
	...
	for (const int& key : wordBitVec)
		fmt::print("{} ", dictionary.get_word(key).value());
	fmt::print("\n");
